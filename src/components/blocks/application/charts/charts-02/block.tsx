...
import {
 Box, Card, FormatNumber, HStack, Show, SimpleGrid, Span, Stat 
...
export const Block = () => {
    ...
    const chart = useChart({
        data,
        series: [{
 name: "value", color: "teal.solid", label 
}],
    })
                            ...
                            <Stat.Root key={summary.id} cursor="pointer" onClick={() => setActive(summary.id)}>
                                ...
                                <HStack gap="5">
                                    ...
                                    <HStack gap="0">
                                        ...
                                        <Span
                                            color={summary.diff > 0 ? "fg.success" : "fg.error"}
                                            fontWeight="medium"
                                            textStyle="xs"
                                        >
                                            <FormatNumber value={summary.diff} style="percent" maximumFractionDigits={1} />
                                        </Span>
                                    </HStack>
                                </HStack>
                            </Stat.Root>
